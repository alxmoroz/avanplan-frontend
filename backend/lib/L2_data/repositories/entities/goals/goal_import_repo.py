...
class GoalImportRepo(EntityRepo):
    def __init__(self):
        super().__init__(
    ...
    def entity_from_schema(self, s: GoalImportSchemaGet) -> GoalImport | None:
        if s:
            g: GoalImport = super().entity_from_schema(s)
            g.parent = self.entity_from_schema(s.parent)
            return g
