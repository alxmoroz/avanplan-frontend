...
from lib.L1_domain.usecases.goal_statuses_uc import GoalStatusesUC
...
def _goal_statuses_uc(
    ...
    uc.get_active_user()
    return GoalStatusesUC(
        g_status_db_repo=dbr.GoalStatusRepo(db),
        g_status_e_repo=er.GoalStatusRepo(),
    )


@router.get("/", response_model=list[GoalStatusSchemaGet])
def get_goals_statuses(
    uc: GoalStatusesUC = Depends(_goal_statuses_uc),
) -> list[GoalStatus]:
    return uc.get_goal_statuses()
