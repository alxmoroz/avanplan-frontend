...
from lib.tests.models.test_task import tmp_object

_tasks_api_path = f"{settings.API_PATH}{router.prefix}/tasks"


def test_get_tasks(client: TestClient, auth_headers_test_user, task_repo):
    with tmp_object(task_repo) as task:
        r = client.get(_tasks_api_path, headers=auth_headers_test_user)
        assert r.status_code == 200
        json_tasks = r.json()
        tasks_out = [Task(**json_task) for json_task in json_tasks]
        assert task in tasks_out
