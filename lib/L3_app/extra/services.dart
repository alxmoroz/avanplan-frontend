...
void setup() {
  // device
  getIt.registerSingletonAsync<BaseDeviceInfo>(
      () async => await DeviceInfoPlugin().deviceInfo);
  getIt.registerSingletonAsync<PackageInfo>(
      () async => await PackageInfo.fromPlatform());

  // repo / adapters
  getIt.registerSingletonAsync<HiveStorage>(
      () async => await HiveStorage().init());

  getIt.registerSingletonAsync<SettingsController>(
      () async => SettingsController().init(),
      dependsOn: [HiveStorage, PackageInfo]);

  // Openapi
  getIt.registerSingletonAsync<Openapi>(
      () async => await setupApi([loaderController.interceptor]),
      dependsOn: [SettingsController]);

  // use cases
  getIt.registerSingletonAsync<AuthUC>(
      () async => await AuthUC(
            emailRepo: AuthEmailRepo(),
  ...
  getIt.registerSingletonAsync<MyUC>(() async => await MyUC(MyRepo()).init(),
      dependsOn: [Openapi]);
  getIt.registerSingletonAsync<TaskUC>(
      () async => await TaskUC(TaskRepo()).init(),
      dependsOn: [Openapi]);
  getIt.registerSingletonAsync<SourceUC>(
      () async => await SourceUC(SourceRepo()).init(),
      dependsOn: [Openapi]);
  getIt.registerSingletonAsync<ImportUC>(
      () async => await ImportUC(ImportRepo()).init(),
      dependsOn: [Openapi]);
  getIt.registerSingletonAsync<AppSettingsUC>(
      () async => await AppSettingsUC(AppSettingsRepo()).init(),
      dependsOn: [Openapi]);
  getIt.registerSingletonAsync<InvitationUC>(
      () async => await InvitationUC(InvitationRepo()).init(),
      dependsOn: [Openapi]);
  getIt.registerSingletonAsync<TaskMemberRoleUC>(
      () async => await TaskMemberRoleUC(TaskMemberRoleRepo()).init(),
      dependsOn: [Openapi]);
  getIt.registerSingletonAsync<PaymentUC>(
      () async => await PaymentUC(PaymentRepo()).init(),
      dependsOn: [Openapi]);
  getIt.registerSingletonAsync<TariffUC>(
      () async => await TariffUC(TariffRepo()).init(),
      dependsOn: [Openapi]);
  getIt.registerSingletonAsync<ContractUC>(
      () async => await ContractUC(ContractRepo()).init(),
      dependsOn: [Openapi]);

  getIt
      .registerSingleton<LocalSettingsUC>(LocalSettingsUC(LocalSettingsRepo()));

  // global state controllers
  getIt.registerSingletonAsync<AuthController>(
      () async => AuthController().init(),
      dependsOn: [HiveStorage, AuthUC]);
