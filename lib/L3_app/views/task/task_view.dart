...
class TaskRoute extends MTRoute {
  ...
  // QuizController может быть у проекта или цели
  AbstractTaskQuizController? qController(TaskController tc, GoRouterState state) {
    AbstractTaskQuizController? qController;
    if (tc.task!.isProject) {
      qController = (state.extra as CreateProjectQuizController?) ?? CreateProjectQuizController(tc);
    } else if (tc.task!.isGoal) {
      qController = (state.extra as CreateGoalQuizController?) ?? CreateGoalQuizController(tc);
    }
    return qController;
  }
  ...
  GoRouterWidgetBuilder? get builder => (context, state) {
        ...
        if (t != null) {
          ...
          final tc = TaskController(t, isNew: isNew);
          tc.loadTask();
          if (isNew) {
            final qc = qController(tc, state);
            if (qc != null) {
              return CreateTaskQuizView(qc);
            }
          }
          return TaskView(tc);
        }
