# avanplan_api (EXPERIMENTAL)
No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

This Dart package is automatically generated by the [OpenAPI Generator](https://openapi-generator.tech) project:

- API version: 1.0
- Generator version: 7.9.0
- Build package: org.openapitools.codegen.languages.DartDioClientCodegen

## Requirements

* Dart 2.15.0+ or Flutter 2.8.0+
* Dio 5.0.0+ (https://pub.dev/packages/dio)

## Installation & Usage

### pub.dev
To use the package from [pub.dev](https://pub.dev), please include the following in pubspec.yaml
```yaml
dependencies:
  avanplan_api: 1.0.0
```

### Github
If this Dart package is published to Github, please include the following in pubspec.yaml
```yaml
dependencies:
  avanplan_api:
    git:
      url: https://github.com/GIT_USER_ID/GIT_REPO_ID.git
      #ref: main
```

### Local development
To use the package from your local drive, please include the following in pubspec.yaml
```yaml
dependencies:
  avanplan_api:
    path: /path/to/avanplan_api
```

## Getting Started

Please follow the [installation procedure](#installation--usage) and then run the following:

```dart
import 'package:avanplan_api/avanplan_api.dart';


final api = AvanplanApi().getAuthApi();
final BodyAuthAppleToken bodyAuthAppleToken = ; // BodyAuthAppleToken | 

try {
    final response = await api.authAppleToken(bodyAuthAppleToken);
    print(response);
} catch on DioException (e) {
    print("Exception when calling AuthApi->authAppleToken: $e\n");
}

```

## Documentation for API Endpoints

All URIs are relative to */api*

Class | Method | HTTP request | Description
------------ | ------------- | ------------- | -------------
[*AuthApi*](doc/AuthApi.md) | [**authAppleToken**](doc/AuthApi.md#authappletoken) | **POST** /v1/auth/apple_token | Apple Token
[*AuthApi*](doc/AuthApi.md) | [**authGoogleToken**](doc/AuthApi.md#authgoogletoken) | **POST** /v1/auth/google_token | Google Token
[*AuthApi*](doc/AuthApi.md) | [**authYandexToken**](doc/AuthApi.md#authyandextoken) | **POST** /v1/auth/yandex_token | Yandex Token
[*AuthApi*](doc/AuthApi.md) | [**passwordToken**](doc/AuthApi.md#passwordtoken) | **POST** /v1/auth/password_token | Password Token
[*AuthApi*](doc/AuthApi.md) | [**refreshToken**](doc/AuthApi.md#refreshtoken) | **POST** /v1/auth/refresh_token | Refresh Token
[*AuthApi*](doc/AuthApi.md) | [**registrationToken**](doc/AuthApi.md#registrationtoken) | **POST** /v1/auth/registration_token | Registration Token
[*AuthApi*](doc/AuthApi.md) | [**requestRegistration**](doc/AuthApi.md#requestregistration) | **POST** /v1/auth/registration | Request Registration
[*MyAccountApi*](doc/MyAccountApi.md) | [**accountV1MyAccountGet**](doc/MyAccountApi.md#accountv1myaccountget) | **GET** /v1/my/account | Account
[*MyAccountApi*](doc/MyAccountApi.md) | [**deleteAccountV1MyAccountDelete**](doc/MyAccountApi.md#deleteaccountv1myaccountdelete) | **DELETE** /v1/my/account | Delete Account
[*MyActivitiesApi*](doc/MyActivitiesApi.md) | [**registerV1MyActivitiesRegisterPost**](doc/MyActivitiesApi.md#registerv1myactivitiesregisterpost) | **POST** /v1/my/activities/register | Register
[*MyAvatarApi*](doc/MyAvatarApi.md) | [**deleteAvatar**](doc/MyAvatarApi.md#deleteavatar) | **DELETE** /v1/my/avatar | Delete Avatar
[*MyAvatarApi*](doc/MyAvatarApi.md) | [**uploadAvatar**](doc/MyAvatarApi.md#uploadavatar) | **POST** /v1/my/avatar | Upload Avatar
[*MyCalendarApi*](doc/MyCalendarApi.md) | [**myCalendarSources**](doc/MyCalendarApi.md#mycalendarsources) | **GET** /v1/my/calendar/sources | Sources
[*MyCalendarApi*](doc/MyCalendarApi.md) | [**myCalendarSourcesUpsert**](doc/MyCalendarApi.md#mycalendarsourcesupsert) | **POST** /v1/my/calendar/sources | Upsert
[*MyCalendarApi*](doc/MyCalendarApi.md) | [**myCalendarsEvents**](doc/MyCalendarApi.md#mycalendarsevents) | **GET** /v1/my/calendar/calendars_events | Calendars Events
[*MyContactsApi*](doc/MyContactsApi.md) | [**deleteMyContact**](doc/MyContactsApi.md#deletemycontact) | **DELETE** /v1/my/contacts/{user_contact_id} | Delete My Contact
[*MyContactsApi*](doc/MyContactsApi.md) | [**myContacts**](doc/MyContactsApi.md#mycontacts) | **GET** /v1/my/contacts | My Contacts
[*MyContactsApi*](doc/MyContactsApi.md) | [**upsertMyContact**](doc/MyContactsApi.md#upsertmycontact) | **POST** /v1/my/contacts | Upsert My Contact
[*MyInvitationsApi*](doc/MyInvitationsApi.md) | [**redeemInvitation**](doc/MyInvitationsApi.md#redeeminvitation) | **POST** /v1/my/invitations/redeem | Redeem Invitation
[*MyNotificationsApi*](doc/MyNotificationsApi.md) | [**markReadV1MyNotificationsPost**](doc/MyNotificationsApi.md#markreadv1mynotificationspost) | **POST** /v1/my/notifications | Mark Read
[*MyNotificationsApi*](doc/MyNotificationsApi.md) | [**myNotificationsV1MyNotificationsGet**](doc/MyNotificationsApi.md#mynotificationsv1mynotificationsget) | **GET** /v1/my/notifications | My Notifications
[*MyPushTokensApi*](doc/MyPushTokensApi.md) | [**updatePushTokenV1MyPushTokensPost**](doc/MyPushTokensApi.md#updatepushtokenv1mypushtokenspost) | **POST** /v1/my/push_tokens | Update Push Token
[*PaymentsApi*](doc/PaymentsApi.md) | [**iapNotificationV1PaymentsIapNotificationPost**](doc/PaymentsApi.md#iapnotificationv1paymentsiapnotificationpost) | **POST** /v1/payments/iap/notification | Iap Notification
[*ProjectMembersApi*](doc/ProjectMembersApi.md) | [**assignProjectMemberRoles_1**](doc/ProjectMembersApi.md#assignprojectmemberroles_1) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/members/{member_id}/roles | Assign Project Member Roles
[*ProjectMembersApi*](doc/ProjectMembersApi.md) | [**projectMemberContacts_1**](doc/ProjectMembersApi.md#projectmembercontacts_1) | **GET** /v1/workspaces/{ws_id}/tasks/{task_id}/members/{member_id}/contacts | Project Member Contacts
[*ProjectStatusesApi*](doc/ProjectStatusesApi.md) | [**deleteStatus_1**](doc/ProjectStatusesApi.md#deletestatus_1) | **DELETE** /v1/workspaces/{ws_id}/tasks/{task_id}/statuses/{status_id} | Delete
[*ProjectStatusesApi*](doc/ProjectStatusesApi.md) | [**statusTasksCount_1**](doc/ProjectStatusesApi.md#statustaskscount_1) | **GET** /v1/workspaces/{ws_id}/tasks/{task_id}/statuses | Status Tasks Count
[*ProjectStatusesApi*](doc/ProjectStatusesApi.md) | [**upsertStatus_1**](doc/ProjectStatusesApi.md#upsertstatus_1) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/statuses | Upsert
[*ReleaseNotesApi*](doc/ReleaseNotesApi.md) | [**releaseNotes**](doc/ReleaseNotesApi.md#releasenotes) | **GET** /v1/release_notes | Release Notes
[*SettingsApi*](doc/SettingsApi.md) | [**settingsV1SettingsGet**](doc/SettingsApi.md#settingsv1settingsget) | **GET** /v1/settings | Settings
[*TaskInvitationsApi*](doc/TaskInvitationsApi.md) | [**createInvitation_1**](doc/TaskInvitationsApi.md#createinvitation_1) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/invitations | Create
[*TaskInvitationsApi*](doc/TaskInvitationsApi.md) | [**getInvitations_1**](doc/TaskInvitationsApi.md#getinvitations_1) | **GET** /v1/workspaces/{ws_id}/tasks/{task_id}/invitations | Invitations
[*TaskNotesApi*](doc/TaskNotesApi.md) | [**deleteNote_1**](doc/TaskNotesApi.md#deletenote_1) | **DELETE** /v1/workspaces/{ws_id}/tasks/{task_id}/notes/{note_id} | Delete
[*TaskNotesApi*](doc/TaskNotesApi.md) | [**uploadAttachment_1**](doc/TaskNotesApi.md#uploadattachment_1) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/notes/{note_id}/attachments | Upload Attachment
[*TaskNotesApi*](doc/TaskNotesApi.md) | [**upsertNote_1**](doc/TaskNotesApi.md#upsertnote_1) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/notes | Upsert
[*TaskRepeatsApi*](doc/TaskRepeatsApi.md) | [**deleteRepeat_1**](doc/TaskRepeatsApi.md#deleterepeat_1) | **DELETE** /v1/workspaces/{ws_id}/tasks/{task_id}/repeats/{repeat_id} | Delete
[*TaskRepeatsApi*](doc/TaskRepeatsApi.md) | [**upsertRepeat_1**](doc/TaskRepeatsApi.md#upsertrepeat_1) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/repeats | Upsert
[*TaskTransactionsApi*](doc/TaskTransactionsApi.md) | [**deleteTransaction_1**](doc/TaskTransactionsApi.md#deletetransaction_1) | **DELETE** /v1/workspaces/{ws_id}/tasks/{task_id}/transactions/{transaction_id} | Delete
[*TaskTransactionsApi*](doc/TaskTransactionsApi.md) | [**upsertTransaction_1**](doc/TaskTransactionsApi.md#upserttransaction_1) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/transactions | Upsert
[*WSMyApi*](doc/WSMyApi.md) | [**myProjects_0**](doc/WSMyApi.md#myprojects_0) | **GET** /v1/workspaces/{ws_id}/my/projects | Projects
[*WSMyApi*](doc/WSMyApi.md) | [**myTasks_0**](doc/WSMyApi.md#mytasks_0) | **GET** /v1/workspaces/{ws_id}/my/tasks | Tasks
[*WSRelationsApi*](doc/WSRelationsApi.md) | [**deleteRelation_0**](doc/WSRelationsApi.md#deleterelation_0) | **DELETE** /v1/workspaces/{ws_id}/relations/{relation_id} | Delete Relation
[*WSRelationsApi*](doc/WSRelationsApi.md) | [**sourcesForRelations_0**](doc/WSRelationsApi.md#sourcesforrelations_0) | **GET** /v1/workspaces/{ws_id}/relations/sources | Sources For Relations
[*WSRelationsApi*](doc/WSRelationsApi.md) | [**upsertRelation_0**](doc/WSRelationsApi.md#upsertrelation_0) | **POST** /v1/workspaces/{ws_id}/relations | Upsert Relation
[*WSSourcesApi*](doc/WSSourcesApi.md) | [**checkConnection_0**](doc/WSSourcesApi.md#checkconnection_0) | **GET** /v1/workspaces/{ws_id}/sources/{source_id}/check_connection | Check Connection
[*WSSourcesApi*](doc/WSSourcesApi.md) | [**deleteSource_0**](doc/WSSourcesApi.md#deletesource_0) | **DELETE** /v1/workspaces/{ws_id}/sources/{source_id} | Delete
[*WSSourcesApi*](doc/WSSourcesApi.md) | [**getProjects_0**](doc/WSSourcesApi.md#getprojects_0) | **GET** /v1/workspaces/{ws_id}/sources/{source_id}/projects | Get Projects
[*WSSourcesApi*](doc/WSSourcesApi.md) | [**requestType_0**](doc/WSSourcesApi.md#requesttype_0) | **POST** /v1/workspaces/{ws_id}/sources/request_type | Request Type
[*WSSourcesApi*](doc/WSSourcesApi.md) | [**startImport_0**](doc/WSSourcesApi.md#startimport_0) | **POST** /v1/workspaces/{ws_id}/sources/{source_id}/start_import | Start Import
[*WSSourcesApi*](doc/WSSourcesApi.md) | [**upsertSource_0**](doc/WSSourcesApi.md#upsertsource_0) | **POST** /v1/workspaces/{ws_id}/sources | Upsert
[*WSTariffOptionsApi*](doc/WSTariffOptionsApi.md) | [**upsertOption_1**](doc/WSTariffOptionsApi.md#upsertoption_1) | **POST** /v1/workspaces/{ws_id}/tariffs/{tariff_id}/options/{option_id} | Upsert
[*WSTariffsApi*](doc/WSTariffsApi.md) | [**availableTariffs_0**](doc/WSTariffsApi.md#availabletariffs_0) | **GET** /v1/workspaces/{ws_id}/tariffs | Available Tariffs
[*WSTariffsApi*](doc/WSTariffsApi.md) | [**sign_0**](doc/WSTariffsApi.md#sign_0) | **POST** /v1/workspaces/{ws_id}/tariffs/{tariff_id}/sign | Sign
[*WSTariffsApi*](doc/WSTariffsApi.md) | [**upsertOption_0**](doc/WSTariffsApi.md#upsertoption_0) | **POST** /v1/workspaces/{ws_id}/tariffs/{tariff_id}/options/{option_id} | Upsert
[*WSTasksApi*](doc/WSTasksApi.md) | [**assignProjectMemberRoles_0**](doc/WSTasksApi.md#assignprojectmemberroles_0) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/members/{member_id}/roles | Assign Project Member Roles
[*WSTasksApi*](doc/WSTasksApi.md) | [**createInvitation_0**](doc/WSTasksApi.md#createinvitation_0) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/invitations | Create
[*WSTasksApi*](doc/WSTasksApi.md) | [**deleteNote_0**](doc/WSTasksApi.md#deletenote_0) | **DELETE** /v1/workspaces/{ws_id}/tasks/{task_id}/notes/{note_id} | Delete
[*WSTasksApi*](doc/WSTasksApi.md) | [**deleteRepeat_0**](doc/WSTasksApi.md#deleterepeat_0) | **DELETE** /v1/workspaces/{ws_id}/tasks/{task_id}/repeats/{repeat_id} | Delete
[*WSTasksApi*](doc/WSTasksApi.md) | [**deleteStatus_0**](doc/WSTasksApi.md#deletestatus_0) | **DELETE** /v1/workspaces/{ws_id}/tasks/{task_id}/statuses/{status_id} | Delete
[*WSTasksApi*](doc/WSTasksApi.md) | [**deleteTask_0**](doc/WSTasksApi.md#deletetask_0) | **DELETE** /v1/workspaces/{ws_id}/tasks/{task_id} | Delete
[*WSTasksApi*](doc/WSTasksApi.md) | [**deleteTransaction_0**](doc/WSTasksApi.md#deletetransaction_0) | **DELETE** /v1/workspaces/{ws_id}/tasks/{task_id}/transactions/{transaction_id} | Delete
[*WSTasksApi*](doc/WSTasksApi.md) | [**duplicateTask_0**](doc/WSTasksApi.md#duplicatetask_0) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/duplicate | Duplicate
[*WSTasksApi*](doc/WSTasksApi.md) | [**getInvitations_0**](doc/WSTasksApi.md#getinvitations_0) | **GET** /v1/workspaces/{ws_id}/tasks/{task_id}/invitations | Invitations
[*WSTasksApi*](doc/WSTasksApi.md) | [**moveTask_0**](doc/WSTasksApi.md#movetask_0) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/move | Move
[*WSTasksApi*](doc/WSTasksApi.md) | [**projectMemberContacts_0**](doc/WSTasksApi.md#projectmembercontacts_0) | **GET** /v1/workspaces/{ws_id}/tasks/{task_id}/members/{member_id}/contacts | Project Member Contacts
[*WSTasksApi*](doc/WSTasksApi.md) | [**repeatTask_0**](doc/WSTasksApi.md#repeattask_0) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/repeat | Repeat
[*WSTasksApi*](doc/WSTasksApi.md) | [**statusTasksCount_0**](doc/WSTasksApi.md#statustaskscount_0) | **GET** /v1/workspaces/{ws_id}/tasks/{task_id}/statuses | Status Tasks Count
[*WSTasksApi*](doc/WSTasksApi.md) | [**taskNode_0**](doc/WSTasksApi.md#tasknode_0) | **GET** /v1/workspaces/{ws_id}/tasks/{task_id} | Task Node
[*WSTasksApi*](doc/WSTasksApi.md) | [**tasksList_0**](doc/WSTasksApi.md#taskslist_0) | **POST** /v1/workspaces/{ws_id}/tasks/list | Tasks List
[*WSTasksApi*](doc/WSTasksApi.md) | [**unlinkTask_0**](doc/WSTasksApi.md#unlinktask_0) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/unlink | Unlink
[*WSTasksApi*](doc/WSTasksApi.md) | [**uploadAttachment_0**](doc/WSTasksApi.md#uploadattachment_0) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/notes/{note_id}/attachments | Upload Attachment
[*WSTasksApi*](doc/WSTasksApi.md) | [**upsertNote_0**](doc/WSTasksApi.md#upsertnote_0) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/notes | Upsert
[*WSTasksApi*](doc/WSTasksApi.md) | [**upsertRepeat_0**](doc/WSTasksApi.md#upsertrepeat_0) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/repeats | Upsert
[*WSTasksApi*](doc/WSTasksApi.md) | [**upsertStatus_0**](doc/WSTasksApi.md#upsertstatus_0) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/statuses | Upsert
[*WSTasksApi*](doc/WSTasksApi.md) | [**upsertTask_0**](doc/WSTasksApi.md#upserttask_0) | **POST** /v1/workspaces/{ws_id}/tasks | Upsert
[*WSTasksApi*](doc/WSTasksApi.md) | [**upsertTransaction_0**](doc/WSTasksApi.md#upserttransaction_0) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/transactions | Upsert
[*WSTransferApi*](doc/WSTransferApi.md) | [**createFromTemplate_0**](doc/WSTransferApi.md#createfromtemplate_0) | **POST** /v1/workspaces/{ws_id}/transfer/create_from_template | Create From Template
[*WSTransferApi*](doc/WSTransferApi.md) | [**destinationsForMove_0**](doc/WSTransferApi.md#destinationsformove_0) | **GET** /v1/workspaces/{ws_id}/transfer/destinations_for_move | Destinations For Move
[*WSTransferApi*](doc/WSTransferApi.md) | [**projectTemplates_0**](doc/WSTransferApi.md#projecttemplates_0) | **GET** /v1/workspaces/{ws_id}/transfer/project_templates | Project Templates
[*WSTransferApi*](doc/WSTransferApi.md) | [**sourcesForMoveTasks_0**](doc/WSTransferApi.md#sourcesformovetasks_0) | **GET** /v1/workspaces/{ws_id}/transfer/sources_for_move | Sources For Move
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**assignProjectMemberRoles**](doc/WorkspacesApi.md#assignprojectmemberroles) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/members/{member_id}/roles | Assign Project Member Roles
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**availableTariffs**](doc/WorkspacesApi.md#availabletariffs) | **GET** /v1/workspaces/{ws_id}/tariffs | Available Tariffs
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**checkConnection**](doc/WorkspacesApi.md#checkconnection) | **GET** /v1/workspaces/{ws_id}/sources/{source_id}/check_connection | Check Connection
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**createFromTemplate**](doc/WorkspacesApi.md#createfromtemplate) | **POST** /v1/workspaces/{ws_id}/transfer/create_from_template | Create From Template
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**createInvitation**](doc/WorkspacesApi.md#createinvitation) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/invitations | Create
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**deleteNote**](doc/WorkspacesApi.md#deletenote) | **DELETE** /v1/workspaces/{ws_id}/tasks/{task_id}/notes/{note_id} | Delete
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**deleteRelation**](doc/WorkspacesApi.md#deleterelation) | **DELETE** /v1/workspaces/{ws_id}/relations/{relation_id} | Delete Relation
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**deleteRepeat**](doc/WorkspacesApi.md#deleterepeat) | **DELETE** /v1/workspaces/{ws_id}/tasks/{task_id}/repeats/{repeat_id} | Delete
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**deleteSource**](doc/WorkspacesApi.md#deletesource) | **DELETE** /v1/workspaces/{ws_id}/sources/{source_id} | Delete
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**deleteStatus**](doc/WorkspacesApi.md#deletestatus) | **DELETE** /v1/workspaces/{ws_id}/tasks/{task_id}/statuses/{status_id} | Delete
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**deleteTask**](doc/WorkspacesApi.md#deletetask) | **DELETE** /v1/workspaces/{ws_id}/tasks/{task_id} | Delete
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**deleteTransaction**](doc/WorkspacesApi.md#deletetransaction) | **DELETE** /v1/workspaces/{ws_id}/tasks/{task_id}/transactions/{transaction_id} | Delete
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**destinationsForMove**](doc/WorkspacesApi.md#destinationsformove) | **GET** /v1/workspaces/{ws_id}/transfer/destinations_for_move | Destinations For Move
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**duplicateTask**](doc/WorkspacesApi.md#duplicatetask) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/duplicate | Duplicate
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**getInvitations**](doc/WorkspacesApi.md#getinvitations) | **GET** /v1/workspaces/{ws_id}/tasks/{task_id}/invitations | Invitations
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**getMyWorkspaces**](doc/WorkspacesApi.md#getmyworkspaces) | **GET** /v1/workspaces | My Workspaces
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**getProjects**](doc/WorkspacesApi.md#getprojects) | **GET** /v1/workspaces/{ws_id}/sources/{source_id}/projects | Get Projects
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**getWorkspace**](doc/WorkspacesApi.md#getworkspace) | **GET** /v1/workspaces/{ws_id} | Get Workspace
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**moveTask**](doc/WorkspacesApi.md#movetask) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/move | Move
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**myProjects**](doc/WorkspacesApi.md#myprojects) | **GET** /v1/workspaces/{ws_id}/my/projects | Projects
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**myTasks**](doc/WorkspacesApi.md#mytasks) | **GET** /v1/workspaces/{ws_id}/my/tasks | Tasks
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**projectMemberContacts**](doc/WorkspacesApi.md#projectmembercontacts) | **GET** /v1/workspaces/{ws_id}/tasks/{task_id}/members/{member_id}/contacts | Project Member Contacts
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**projectTemplates**](doc/WorkspacesApi.md#projecttemplates) | **GET** /v1/workspaces/{ws_id}/transfer/project_templates | Project Templates
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**repeatTask**](doc/WorkspacesApi.md#repeattask) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/repeat | Repeat
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**requestType**](doc/WorkspacesApi.md#requesttype) | **POST** /v1/workspaces/{ws_id}/sources/request_type | Request Type
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**sign**](doc/WorkspacesApi.md#sign) | **POST** /v1/workspaces/{ws_id}/tariffs/{tariff_id}/sign | Sign
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**sourcesForMoveTasks**](doc/WorkspacesApi.md#sourcesformovetasks) | **GET** /v1/workspaces/{ws_id}/transfer/sources_for_move | Sources For Move
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**sourcesForRelations**](doc/WorkspacesApi.md#sourcesforrelations) | **GET** /v1/workspaces/{ws_id}/relations/sources | Sources For Relations
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**startImport**](doc/WorkspacesApi.md#startimport) | **POST** /v1/workspaces/{ws_id}/sources/{source_id}/start_import | Start Import
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**statusTasksCount**](doc/WorkspacesApi.md#statustaskscount) | **GET** /v1/workspaces/{ws_id}/tasks/{task_id}/statuses | Status Tasks Count
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**taskNode**](doc/WorkspacesApi.md#tasknode) | **GET** /v1/workspaces/{ws_id}/tasks/{task_id} | Task Node
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**tasksList**](doc/WorkspacesApi.md#taskslist) | **POST** /v1/workspaces/{ws_id}/tasks/list | Tasks List
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**unlinkTask**](doc/WorkspacesApi.md#unlinktask) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/unlink | Unlink
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**updateWorkspace**](doc/WorkspacesApi.md#updateworkspace) | **POST** /v1/workspaces/{ws_id} | Upsert
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**uploadAttachment**](doc/WorkspacesApi.md#uploadattachment) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/notes/{note_id}/attachments | Upload Attachment
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**upsertNote**](doc/WorkspacesApi.md#upsertnote) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/notes | Upsert
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**upsertOption**](doc/WorkspacesApi.md#upsertoption) | **POST** /v1/workspaces/{ws_id}/tariffs/{tariff_id}/options/{option_id} | Upsert
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**upsertRelation**](doc/WorkspacesApi.md#upsertrelation) | **POST** /v1/workspaces/{ws_id}/relations | Upsert Relation
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**upsertRepeat**](doc/WorkspacesApi.md#upsertrepeat) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/repeats | Upsert
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**upsertSource**](doc/WorkspacesApi.md#upsertsource) | **POST** /v1/workspaces/{ws_id}/sources | Upsert
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**upsertStatus**](doc/WorkspacesApi.md#upsertstatus) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/statuses | Upsert
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**upsertTask**](doc/WorkspacesApi.md#upserttask) | **POST** /v1/workspaces/{ws_id}/tasks | Upsert
[*WorkspacesApi*](doc/WorkspacesApi.md) | [**upsertTransaction**](doc/WorkspacesApi.md#upserttransaction) | **POST** /v1/workspaces/{ws_id}/tasks/{task_id}/transactions | Upsert


## Documentation For Models

 - [AppSettingsGet](doc/AppSettingsGet.md)
 - [AttachmentGet](doc/AttachmentGet.md)
 - [AuthToken](doc/AuthToken.md)
 - [BodyAuthAppleToken](doc/BodyAuthAppleToken.md)
 - [BodyAuthGoogleToken](doc/BodyAuthGoogleToken.md)
 - [BodyAuthYandexToken](doc/BodyAuthYandexToken.md)
 - [BodyIapNotificationV1PaymentsIapNotificationPost](doc/BodyIapNotificationV1PaymentsIapNotificationPost.md)
 - [BodyMyCalendarSourcesUpsert](doc/BodyMyCalendarSourcesUpsert.md)
 - [BodyRedeemInvitation](doc/BodyRedeemInvitation.md)
 - [BodyRegisterV1MyActivitiesRegisterPost](doc/BodyRegisterV1MyActivitiesRegisterPost.md)
 - [BodyRegistrationToken](doc/BodyRegistrationToken.md)
 - [BodyRequestRegistration](doc/BodyRequestRegistration.md)
 - [BodyRequestType](doc/BodyRequestType.md)
 - [BodyStartImport](doc/BodyStartImport.md)
 - [BodyUpdatePushTokenV1MyPushTokensPost](doc/BodyUpdatePushTokenV1MyPushTokensPost.md)
 - [CalendarEventAttendee](doc/CalendarEventAttendee.md)
 - [CalendarEventGet](doc/CalendarEventGet.md)
 - [CalendarGet](doc/CalendarGet.md)
 - [CalendarSourceGet](doc/CalendarSourceGet.md)
 - [CalendarsEvents](doc/CalendarsEvents.md)
 - [ContractGet](doc/ContractGet.md)
 - [EstimateUnitGet](doc/EstimateUnitGet.md)
 - [EstimateValueGet](doc/EstimateValueGet.md)
 - [HTTPValidationError](doc/HTTPValidationError.md)
 - [Invitation](doc/Invitation.md)
 - [InvitationGet](doc/InvitationGet.md)
 - [InvoiceDetailGet](doc/InvoiceDetailGet.md)
 - [InvoiceGet](doc/InvoiceGet.md)
 - [MemberContactGet](doc/MemberContactGet.md)
 - [MemberGet](doc/MemberGet.md)
 - [MyUser](doc/MyUser.md)
 - [NoteGet](doc/NoteGet.md)
 - [NoteUpsert](doc/NoteUpsert.md)
 - [Notification](doc/Notification.md)
 - [PermissionGet](doc/PermissionGet.md)
 - [PermissionRoleGet](doc/PermissionRoleGet.md)
 - [ProjectGet](doc/ProjectGet.md)
 - [ProjectStatusGet](doc/ProjectStatusGet.md)
 - [ProjectStatusUpsert](doc/ProjectStatusUpsert.md)
 - [PromoActionGet](doc/PromoActionGet.md)
 - [Registration](doc/Registration.md)
 - [ReleaseNoteGet](doc/ReleaseNoteGet.md)
 - [RoleGet](doc/RoleGet.md)
 - [SettingsGet](doc/SettingsGet.md)
 - [SourceGet](doc/SourceGet.md)
 - [SourceUpsert](doc/SourceUpsert.md)
 - [TariffGet](doc/TariffGet.md)
 - [TariffOptionGet](doc/TariffOptionGet.md)
 - [TaskGet](doc/TaskGet.md)
 - [TaskNode](doc/TaskNode.md)
 - [TaskRelationGet](doc/TaskRelationGet.md)
 - [TaskRelationUpsert](doc/TaskRelationUpsert.md)
 - [TaskRemote](doc/TaskRemote.md)
 - [TaskRepeatGet](doc/TaskRepeatGet.md)
 - [TaskRepeatUpsert](doc/TaskRepeatUpsert.md)
 - [TaskSettingsGet](doc/TaskSettingsGet.md)
 - [TaskSource](doc/TaskSource.md)
 - [TaskSourceGet](doc/TaskSourceGet.md)
 - [TaskTransactionGet](doc/TaskTransactionGet.md)
 - [TaskTransactionUpsert](doc/TaskTransactionUpsert.md)
 - [TaskUpsert](doc/TaskUpsert.md)
 - [TasksChanges](doc/TasksChanges.md)
 - [UActivityGet](doc/UActivityGet.md)
 - [User](doc/User.md)
 - [UserContactGet](doc/UserContactGet.md)
 - [UserContactUpsert](doc/UserContactUpsert.md)
 - [ValidationError](doc/ValidationError.md)
 - [ValidationErrorLocInner](doc/ValidationErrorLocInner.md)
 - [WorkspaceGet](doc/WorkspaceGet.md)
 - [WorkspaceUpsert](doc/WorkspaceUpsert.md)


## Documentation For Authorization


Authentication schemes defined for the API:
### APIKeyHeader

- **Type**: API key
- **API key parameter name**: Avanplan
- **Location**: HTTP header

### OAuth2PasswordBearer

- **Type**: OAuth
- **Flow**: password
- **Authorization URL**: 
- **Scopes**: N/A


## Author



